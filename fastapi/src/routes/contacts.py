...
async def upcoming_birthdays(days: int = 7, db: AsyncSession = Depends(get_db)):
    contacts = await repo_contact.get_upcoming_birthdays(db, days)
    return contacts


@router.get("/{contact_id}", response_model=ContactResponce)
async def get_contact(contact_id: int = Path(ge=1), db: AsyncSession = Depends(get_db)):
    contact = await repo_contact.get_contact(contact_id, db)
    if contact is None:
        raise HTTPException(
...
async def get_contacts(
    ...
    contacts = await repo_contact.get_contacts(limit, offset, db)
    return contacts


@router.post("/", response_model=ContactResponce, status_code=status.HTTP_201_CREATED)
async def create_contact(body: ContactSchema, db: AsyncSession = Depends(get_db)):
    contact = await repo_contact.create_contact(body, db)
    return contact
...
async def update_contact(
    ...
    contact = await repo_contact.update_contact(contact_id, body, db)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
...
async def delete_contact(
    ...
    contact = await repo_contact.delete_contact(contact_id, db)
    return contact
